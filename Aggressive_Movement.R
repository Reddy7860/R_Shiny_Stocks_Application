...
execute_orders <- function(order_data,execute_df){
  ...
  for(i in 1:nrow(execute_df)){
    ...
    if(execute_df[i,"exec_rnk"] == 1){
      ...
      if((as.numeric(difftime(Sys.time(), as.POSIXct(execute_df[i,"Datetime"], origin="1970-01-01"), units ="mins")) <= 20)){
        ...
        }else if(execute_df[i,"order_place"] > 1){
          
          if(execute_df[i,"target_order_id"] == 0){
          # if(execute_df[i,"robo_order_id"] == 0){
            ...
            if(nrow(last_order) > 0 ){
              ...
              if(last_order$status == "complete"){
                ...
                prev_generation <- execute_df[(i),]
                
                # print(prev_generation)
                
                prev_trading_symbol = nifty_50_data[nifty_50_data$Yahoo.Symbol==as.character(execute_df[(i),2]),]$TradingSymbol
                prev_symbol_token = nifty_50_data[nifty_50_data$Yahoo.Symbol==as.character(execute_df[(i),2]),]$Symbol.Token
                
                prev_transaction_type = as.character(execute_df[(i),3])
                prev_exchange_counter = "NSE"
                prev_order_type = "LIMIT"
                prev_product_type = "INTRADAY"
                prev_duration_day = "DAY"
                prev_stoploss = as.numeric(execute_df[(i),6])
                
                prev_squareoff = as.numeric(execute_df[(i),7])
                # qty = Signal_df[i,8]
                prev_qty = as.numeric(execute_df[(i),8])
                prev_limit_price = as.numeric((execute_df[(i),5]))
                ...
                # robo_order_place <- place_order(object = session_data,
                #                            variety= "ROBO",
                ...
                #                            price= NULL,
                #                            quantity= prev_qty,
                #                            squareoff= as.character(abs(prev_limit_price - prev_squareoff)),
                #                            stoploss= as.character(abs(prev_limit_price - prev_stoploss))
                # )
      ...
      }else if((as.numeric(difftime(Sys.time(), as.POSIXct(execute_df[i,"Datetime"], origin="1970-01-01"), units ="mins")) >= 30)){
          ...
          if(prev_order_id > 0){
            ...
            if(execute_df[i,"cancel_order_id"] == 0){
              ...
              if(last_order$status == "open"){
                ...
                Sys.sleep(1)
                # print(cancel_id)
                execute_df[(i),"cancel_order_id"] <- cancel_id
                execute_df[(i),"conclusion"] <- "Cancelled Order"
              }
      ...
      if(execute_df[i,"order_place"] == 0){
        ...
        if(nrow(last_order) > 0 ){
          ...
          if(last_order$status == "complete"){
            ...
            #######   Checking if the previous Robo order is placed
            if(execute_df[i,"target_order_id"] == 0){
              
            # if(execute_df[(i-1),"robo_order_id"] == 0){
            ...
            # execute_df[(i-1),"robo_order_id"] <- robo_order_place
            execute_df[(i-1),"target_place"] <- 1
            execute_df[(i-1),"conclusion"] <- "Target Pending"
            # execute_df[(i-1),"conclusion"] <- "Robo Order Pending"
          ...
          if(last_order$status == "open" || (as.numeric(difftime(Sys.time(), as.POSIXct(execute_df[(i-1),"Datetime"], origin="1970-01-01"), units ="mins")) >= 30)){
            
            prev_order_id <- last_order$order_id
      ...
      # browser()
      # if(1==1){
      if((as.numeric(difftime(Sys.time(), as.POSIXct(execute_df[(i),"Datetime"], origin="1970-01-01"), units ="mins")) <= 100)){
...
nifty50_data <- read.csv("~/Desktop/Reddy_Stocks_Application/data/Nifty50_Stocks.csv")

temp_df = c("BAJFINANCE.NS","TCS.NS","GRASIM.NS","TITAN.NS","BAJAJFINSV.NS")

final_conbined_signals = data.frame(
  ...
  Target = as.character(),
  Qty = as.character(),
  exec_rnk = as.character(),
  order_place = as.character(),
  order_id = as.character(),
  target_order_id = as.character(),
  cancel_order_id = as.character(),
  robo_order_id = as.character(),
  conclusion = as.character()
...
if(nrow(Signal_df)> 0 ){
  ...
  final_signals_df <- sqldf("select COALESCE(tsd.Strategy,sd.Strategy) as Strategy,
                           ...
                           from Signal_df sd
                           left join (select * from temp_signals_df where rnk =1) tsd on sd.Strategy = tsd.Strategy and sd.Stock = tsd.Stock and sd.Datetime = tsd.Datetime
                          ")
  ...
  tryCatch(
    {
      ...
      orders_df <- execute_orders(order_data,final_signals_df)
      print(orders_df)
    ...
    warning=function(cond) {
      ...
      # print(orders_df)
      final_conbined_signals <- rbind(final_conbined_signals,orders_df)
      # print(Signal_df)
      # print(final_conbined_signals)
      if(nrow(orders_df) > 0){
        orders_df$execution_time = Sys.time()
        write.table(orders_df, file = "~/Desktop/Reddy_Stocks_Application/data/Aggressive_Movement.csv", sep = ",",row.names = FALSE,append = T,col.names = FALSE)
      }
      # write.table(final_conbined_signals, "~/Desktop/Reddy_Stocks_Application/data/Aggressive_Movement.csv", sep = ",", col.names = !file.exists("~/Desktop/Reddy_Stocks_Application/data/Aggressive_Movement.csv"), append = T)
      # write.table(orders_df, file = "~/Desktop/Reddy_Stocks_Application/data/Aggressive_Movement.csv", sep = ",",quote="",row.names = FALSE)
    }
  )
}
...
# write.csv(final_conbined_signals,"~/Desktop/Reddy_Stocks_Application/data/Aggressive_Movement.csv")
